    ...
    <form key={subGoal.id} onSubmit={event => {
      ...
      <table>
        <tbody>
          ...
          <tr><td className='label'><label>Completed</label></td><td className='input'><input type='checkbox' defaultChecked={subGoal.completed_on !== null} onChange={event => setCompleted(event.target.checked)}></input></td></tr>
        </tbody>
      </table>
      <footer>
